...
public class TransformDataToDocumentsMain {
  public static void main(String[]args)throws Exception {
    ...
    int ptr = -1;
    int[] prev_full_run_idxs = new int[runs.rows.size()];
    int[] next_full_run_idxs = new int[runs.rows.size()];
    for (int i=0; i<prev_full_run_idxs.length; i++) {
      String[] row = runs.rows.get(i);
...
//      }
    ...
    for (String[] changelist : cls.rows) {
      ...
      String prev_full_run_id = runs.rows.get(prev_full_run_idx)[0];
      String next_full_run_id = runs.rows.get(next_full_run_idx)[0];
